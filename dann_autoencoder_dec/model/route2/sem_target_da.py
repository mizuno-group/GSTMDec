...
class DANN_SEM(nn.Module):
    ...
    def forward(self, source_x, target_x, source_y, dropout_mask, temperature=1.0):
        ...
        # predictor
        source_pred = self.predictor(output['gaussian'][:source_x.shape[0]])
        target_pred = self.predictor(output['gaussian'])[source_x.shape[0]:]  # FIXME: is it necessary to split?
        output['source_pred'] = source_pred
        output['target_pred'] = target_pred

        # discriminator
        domain_source = self.discriminator(output['gaussian'][:source_x.shape[0]])
        domain_target = self.discriminator(output['gaussian'][source_x.shape[0]:])
