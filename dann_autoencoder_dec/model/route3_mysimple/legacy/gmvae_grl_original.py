...
- Gaussian Mixture Variational Autoencoder (GMVAE)
    - Based on NSEM-GMHTM (Original version)
...
class MultiTaskAutoEncoder(nn.Module):
    def __init__(self, option_list, seed=42):
        ...
        # layers
        self.encoder = nn.Sequential(nn.Linear(self.feature_num, self.topic_num_1), nn.BatchNorm1d(self.topic_num_1), nn.Tanh())
    ...
    def forward(self, x, temperature=1.0, alpha=1.0):
        ...
        rec = self.decode(x_ori, out_dec["x_rec1"], out_dec["x_rec2"], out_dec["x_rec3"])
