...
class MultiTaskAutoEncoder(nn.Module):
    def __init__(self, option_list, seed=42):
        ...
        self.inference = InferenceNet(x_dim=1, z_dim=self.latent_dim, y_dim=1, n_gene=self.feature_num, nonLinear=nonLinear)
        self.generative = GenerativeNet(x_dim=1, z_dim=self.latent_dim, y_dim=1, n_gene=self.feature_num, nonLinear=nonLinear)
    ...
    def forward(self, x, alpha=1.0):
        batch_size = x.size(0)
        x = Variable(x.cuda()).view(x.size(0), -1, 1)  # (batch_size, feature_dim) -> (batch_size, feature_dim, 1)
        #x = x.view(batch_size, -1)

        # inference-generative
        out_inf = self.inference(x)
        z, y = out_inf['gaussian'], out_inf['categorical']
        rec = self.generative(z, y)
