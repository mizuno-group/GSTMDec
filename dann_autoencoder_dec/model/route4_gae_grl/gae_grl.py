...
Graph Autoencoder (GAE) with Gradient Reversal Layer (GRL)

References:
- https://github.com/huawei-noah/trustworthyAI/blob/master/gcastle/castle/algorithms/gradient/gae/torch/models/model.py
...
class LossFunctions:
    ...
    def rec_loss(self, real, predicted, dim=-1):
        if dim == -1:
            loss = torch.mean((real - predicted).pow(2))
        else:
            loss = torch.mean((real - predicted).pow(2), dim=dim)
            #loss = torch.mean((real - predicted).pow(2), dim=1)  # row-wise mean
        n = real.shape[0]
        loss = (0.5/n) * loss
...
class MultiTaskAutoEncoder(nn.Module):
    def __init__(self, option_list, W=None, seed=42):
    ...
    def forward(self, x, alpha=1.0):
        ...
        # 2. Linear transformation AT H(j) and Decoder
        self.w_adj = self._preprocess_graph(self.w)
        emb2 = torch.mm(emb, self.w_adj)
        rec = self.decoder(emb2)

        # 3. Predictor
        pred = self.predictor(emb)

        # 4. Domain Discriminator with GRL
