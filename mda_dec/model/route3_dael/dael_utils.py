...
def random_masking(tensor: torch.Tensor, mask_prob: float = 0.1) -> torch.Tensor:
    """
    Randomly mask some elements of the input tensor.
    ...
    Returns:
        torch.Tensor: Tensor after random masking.
    """
    if mask_prob <= 0:
        # If mask probability is zero or negative, return the original tensor
        return tensor

    mask = torch.rand_like(tensor) < mask_prob
    masked_tensor = tensor.clone()
    masked_tensor[mask] = 0  # Set masked elements to zero
