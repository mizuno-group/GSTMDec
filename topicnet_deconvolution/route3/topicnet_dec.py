...
class GBN_model(nn.Module):
    def __init__(self, args):
        ...
        self.wei_shape_max = torch.tensor(1e8).float()  # Original: torch.tensor(10.0).float()
        ...
        elif self.topic_tree_path.endswith('mat'):
            graph_wordnet = sio.loadmat(self.topic_tree_path)
            self.graph = []
