...
class InferenceNet(nn.Module):
    ...
    def forward(self, x, adj, adj_2, adj_3, temperature, hard = 0):
        if inv_flag ==True:
            x_1 = torch.matmul(adj.to(torch.float32),x.squeeze(2).T).T  # FIXME: mat1 and mat2 shapes cannot be multiplied (8x8 and 300x512)
...
class net(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        x_dim, y_dim, z_dim = 64, 10, 10  # x:  y:  z: # FIXME ??
    ...
    def forward(self, x, dropout_mask=None, temperature=1.0, hard=0):
        ...
        out_inf_1 = self.inference(  # FIXME
