...
class LDA:
    def __init__(self, K, alpha, beta, docs, V, smartinit=True):
        ...
        self.n_m_z = numpy.full((len(self.docs), K), alpha)     # word count of each document and topic
        self.n_z_t = numpy.full((K, V), beta) # word count of each topic and vocabulary
        self.n_z = numpy.full(K, V * beta)    # word count of each topic
    ...
    def output_word_topic_dist(self, voca):
        zcount = numpy.zeros(self.K, dtype=int)
