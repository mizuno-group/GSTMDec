...
class AEimpute(object):
    ...
    def train(self, ref_data, target_data):
        ...
        for epoch in range(self.num_epochs):
            ...
            metric_logger['cAE_loss'].append(loss_epoch.cpu().detach().numpy())
            pred_loss_epoch = pred_loss_epoch/(batch_idx + 1)
            metric_logger['pred_loss'].append(pred_loss_epoch.cpu().detach().numpy())
            recon_loss_epoch = recon_loss_epoch/(batch_idx + 1)
            metric_logger['recon_loss'].append(recon_loss_epoch.cpu().detach().numpy())
            if (epoch+1) % 10 == 0:
